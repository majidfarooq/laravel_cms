...
@section('content')
    ...
    <!-- Page Header-->
    <header class="masthead" style="background-image: url('{{(isset($category->image) ? asset($category->image) : asset('/public/storage/placeholder.jpg'))}}')">
            ...
            <div class="row gx-4 gx-lg-5 justify-content-center">
                <div class="col-md-12 col-lg-12 col-xl-12">
                    <span class="meta text-white">
                        ...
                        <small><i class="fas fa-arrow-right"></i></small>
                        <a class="text-white"  href="{{ (isset($category->slug) ? route('category.show',$category->slug) : '') }}">{{ (isset($category->name) ? $category->name : '') }}</a>
                    </span>
                    <div class="text-start site-heading text-uppercase"><h1>{{ (isset($category->name) ? $category->name : '') }}</h1></div>
                </div>
            </div>
        </div>
    </header>
    ...
    <div class="container mb-4">
        <div class="row">
            <div class="col-12">
                <p class="text-center">{{ (isset($category->description) ? $category->description : '') }}</p>
            </div>
        </div>
    </div>
