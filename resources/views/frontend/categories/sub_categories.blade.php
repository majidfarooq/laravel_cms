            ...
            <div class="container-xl">
                @if (isset($categories))
                    @foreach($categories as $category)
                        @if ($category->subCatExperiences->count() !=0)
                            <h3 class="mb-3 mt-3">{{(isset($category->name)?$category->name:'')}}</h3>
                            <div class="row">
                                @php $counter=1 @endphp
                                @foreach($category->subCatExperiences as $experience)

                                @if($counter<6)
                                    @if($counter==1)
                                        <div class="col-lg-4 col-md-12 left-experience experience_{{$counter}}">
                                    @elseif($counter==2)
                                        <div class="col-lg-8 col-md-12 right-experience">
                                        <div class="row">
                                        <div class="col-lg-6 col-md-12 experience_{{$counter}}">
                                    @else
                                        <div class="col-lg-6 col-md-12 experience_{{$counter}}">
                                    @endif
                                ...
                                @endif
                                    <div class="view-more col-12">
                                        <h6>
                                            <a href="{{route('sub.category',$category->slug)}}">View More</a>
                                            <span>
                                                    <img src="{{asset('public/assets/frontend/images/double-arrow.png')}}">
                                                </span>
                                        </h6>
                                    </div>
                            </div>
